...
- `std/prelude` is the prelude described above. It declares nothing itself: it
  ...
  and the receiver scopes that make builtin methods available.
...
the first six rows below in full and the receiver scopes from the following two
rows, while the rest are imported explicitly:
