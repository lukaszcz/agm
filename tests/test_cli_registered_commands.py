...
def test_param_parse_failure_raises_a_typed_usage_error(tmp_path: Path) -> None:
    """``run`` no longer renders a usage message itself: it raises a typed error
    carrying the parse-failure message and param inventory, so the CALLER (a
    plain ``agm exec`` invocation, or registered-command dispatch) can render
    it appropriately. See ``agm.commands.exec`` and ``agm.cli_dispatch``."""
    import agm.commands.exec_program as exec_program
    ...
    with pytest.raises(RegisteredParamUsageError) as exc_info:
        exec_program.run(
            ExecArgs(
                file=str(source),
    ...
    assert [p.name for p in exc_info.value.params] == ["level"]


def test_registered_command_param_error_renders_shared_usage_help(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """A CLI param-parse failure on a dispatched registered command renders
...
def test_registered_command_param_error_handles_no_description_or_params(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    import agm.cli_dispatch as dispatch
...
def test_plain_exec_param_error_still_renders_the_base_exec_usage(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    """A direct ``agm exec FILE`` param-parse failure still renders the
...
def test_registered_params_ignore_a_program_owned_by_another_package(tmp_path: Path) -> None:
