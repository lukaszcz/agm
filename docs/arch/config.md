...
Every non-override stdlib candidate (home, installation prefix, then the repository checkout) carries a top-level `STDLIB_CONTRACT` marker file holding a contract id; a candidate is only selected if its marker matches the id the running code expects, so a candidate left stale (by a manual edit, an interrupted install, an older cross-version tree, or an incompatible public stdlib surface) is skipped in favor of a compatible one rather than silently loaded. When every candidate exists but none matches, resolution fails fast with an error naming the stale path and pointing at `just install`; when no candidate exists at all, the home destination is returned unresolved so diagnostics can name the path `just install` would populate. `just install` also installs and activates the lockstep `std` package in the versioned package store. Unlike `config.toml`, the sandbox templates, and the prompts — which `tools/install_agm_config.py` writes only when absent, preserving user edits — both the installed stdlib compatibility mirror and store tree are managed artifacts: install refuses symlinks anywhere in their destination trees, then force-refreshes and prunes them, so the marker can never certify stale sources.
