...
- **Compositional expansion.** `import foo.* [qualified] [using…|hiding…]` ≡ applying that exact
  clause to *every* module in the subtree: open → each submodule's names unqualified (clash-on-use)
  plus full-path qualified; `qualified` → full-path qualified only; `using`/`hiding` applied
  per-module against each module's own exports.
...
- **Lenient merge.** Multiple `import` declarations for the **same module** union their effects
  (S = union of each declaration's set; all aliases/handles valid; unqualified injection from any
  open declaration). A module's effective import is the union across its declarations.
  Re-importing identically is idempotent.
- **Static errors:**
  - the **same alias** bound to **different** modules (`import foo as A` + `import bar as A`);
...
- **Importable modules are declaration-only**: `def`, `record`, `enum`, `type`, `agent` (with
  optional `private`). Top-level bare expressions, `let`/`var`, `print`/`exec`/`ask`, and agent
  calls are a **scope error** inside an imported module.
...
- `private` declarations are usable within their module but **invisible** to other modules — not
  ...
  the export set. Applies to `def`/`record`/`enum`/`type`/`agent`.

### D6 — Whole-program declarations
- `program`, `config`, and `param` are **entry-file-only**; each is a scope error inside an
  imported module. Libraries parameterize via **function arguments**.

### D7 — Agent identity
- Agents are module-level declarations with **fully-qualified identity** (`foo.bar::reviewer`):
  importable, `private`-able, `using`/`hiding`/wildcard-controllable, renamable. Two modules
  declaring `reviewer` are distinct agents.
- The host **agent registry is keyed by qualified name**; runner specs stay at the declaration
  site (`agent NAME = "..."`); the host builds runners for every declared agent across all
  loaded modules.
...
- **One unified namespace.** Any exported top-level value *or* type may be listed in
  ...
  type from value). **Agents are values.** **Enum variants travel with their enum** — not
...
- `lexer/` — soft keywords `import`, `qualified`, `using`, `hiding`, `private` (`as` exists);
  ...
  `lexer.py`)
- `parser/grammar/agl.lark`, `parser/transform.py` — import decl (incl. `.*` and any-case alias),
  qualified refs, `::name`, alias-rooted multi-segment qualifiers, `private`, module-qualified
  type/constructor heads.
- `syntax/nodes.py`, `syntax/types.py` — `ImportDecl` (with `wildcard: bool`), qualifier on
  refs/constructors/types, `is_private` flags, module-unit / module-graph nodes.
...
- `runtime/agents.py`, `runtime/runtime.py` — `AgentRegistry` keyed by qualified name; new
  load-and-prepare entrypoint (entry source + entry path + roots).
...
- `src/agm/commands/` (exec), `src/agm/cli.py`, `src/agm/completion.py` — `-I/--module-path`
  option + completion.
- Docs: `README.md` (brief), `docs/commands.md` (full), `docs/agl-grammar.md`, help texts.
...
// qualifier before "::": a module path, OR an alias optionally followed by path segments
// (alias-rooted, from wildcard re-rooting), OR empty (current module).
mod_qualifier: module_path | alias ("." VAR_NAME)*
...
- `private` is a leading modifier on the five declaration rules.
...
- `Qualifier{segments: tuple[str, ...], is_alias: bool}` where `segments == ()` = current module
  (`::name`); alias-rooted qualifiers carry the alias + trailing segments. Reference nodes
  (`VarRef`, `Constructor`, type refs) gain optional `qualifier: Qualifier | None`.
- `is_private: bool` on the five declaration node types.
...
- `roots.py`: assemble the **unordered** root set from invocation context + config + `-I` flags.
- `resolver.py`:
  - single id → search all roots for `<root>/<segments>.agl`; **exactly one** match required
    (zero → not-found listing roots; ≥2 → ambiguity error).
...
- `loader.py`:
  ...
  4. Errors: not-found (list roots); ambiguous id; parse error (file + span); attempt to import
...
- Build each module's **top-level environment**: own decls (incl. `private`) + imports. For each
  ...
  for wildcard re-rooting). **Merge** multiple imports of the same module by unioning effects.
- **Reference resolution**:
  ...
  - bare `name` → {current top-level decls, open-imported names, builtins}; multiple distinct
    candidates ⇒ defer, emitting an *ambiguous reference* diagnostic only at an unqualified
    use-site, listing disambiguating qualifiers.
- **Whole-graph pre-passes** (extend existing agent/function pre-passes) collect all modules'
  top-level decls before resolving any body → cross-file mutual recursion (D8).
- **Enforcement**: declaration-only in non-entry modules (D4); `program`/`config`/`param`
  entry-only (D6); `private` boundary (D5); duplicate-alias / alias-root collision static errors
  (D3).
...
- Agent typing uses qualified agent names.

### 8.2 Eval / runtime
- Build **per-module top-level frames** (closures, agent values, type bindings) — pure, no side
  effects. References (statically resolved to `(ModuleId, name)`) fetch from the right frame;
  cross-file mutual recursion works once all frames exist.
- Execute **only the entry module's block**.
- `AgentRegistry` keyed by **qualified name**; runners built from every declared `agent` across
  loaded modules. Update REPL `ConfirmingAgent` wiring.
...
Located messages for: module-not-found (roots searched); ambiguous module id (multiple roots);
...
module; attempt to import the entry; duplicate alias to different modules; alias-equals-module-root
collision.
...
- **Loader/resolver (modules/)**: id→path across the root set; not-found; **ambiguous id across
  roots**; wildcard glob across roots; graph build; dedup; **cycles** terminate & link; node-id
  disjointness.
- **Scope**: open import; `using`/`hiding`/`qualified`/`as`/rename; S bounds qualified; clash
  ...
  compositional expansion + re-rooting; cross-file mutual recursion.
- **Typecheck**: distinct module-qualified type identity; qualified type refs; whole-graph
  pre-pass with cycles.
- **Eval**: qualified value/agent dispatch; cross-module + mutually-recursive calls; qualified
  agent registry keys.
...
- **M1 — Surface syntax**: lexer soft keywords + `.*`; grammar (import decl incl. wildcard &
  ...
  Parser/AST tests only; grammar conflict-free.
...
- **M4 — Typecheck**: module-qualified type identity, qualified type refs, whole-graph type
  pre-pass.
- **M5 — Eval & host**: per-module frames, qualified/mutually-recursive dispatch, qualified agent
  registry, `agm exec` module loading. Eval tests + first multi-file e2e (multi-scenario).
- **M6 — REPL, CLI, docs, polish**: REPL imports, `-I/--module-path` + completion, full
  diagnostics, README / commands.md / grammar.md / help texts, coverage to 100%. Final
  `just check`.
