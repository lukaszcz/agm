...
The parser accepts blocks ending in a `let` or `var` binder and preserves those binders in the AST, including suite blocks and the marked inline bodies used by loops, parenthesized blocks, and `try` bodies. `LetDecl` carries one `Pattern` root (rather than a name); `VarDecl` remains name-bearing. The current compatibility path resolves, checks, and lowers only simple variable and wildcard `let` roots, leaving destructuring-let semantics to the pattern pipeline work. Typechecking gives a trailing binder a unit result (or bottom when its initializer exits); lowering supplies that result without altering the AST.
