...
The fact built from `@command` and its prose attributes is `command_registrations`, one `ProgramCommandSpec` per `program def` that registers itself as a package command, keyed by node id: the command path plus the `@description` and `@help` the registration carries. `@description` and `@help` describe a registration rather than a program, so each is an error without `@command`; the path answers to the same rule a manifest command path does, which the catalog checks against the CLI's own reserved surface. The table is published like any other, but the package domain does not read it: registration discovery runs over a bare parse, and calls `recognize_program_command` to apply this same recognition to one declaration without a scope pass ([packages.md](../../packages.md)). Resolving a package module still validates its `@command` attributes, as it does every other.
