...
class TestLoop:
    ...
    def test_uses_command_specific_loop_config_for_positional_command(
        ...
        work = tmp_path / "work"
        work.mkdir()
        (work / "codex" / "tasks").mkdir(parents=True)
        (work / "codex" / "tasks" / "PROGRESS.md").write_text("started\n")
    ...
    def test_loop_exposes_resolved_tasks_dir_to_selector_and_runner(
        ...
        assert len(selector_log_lines) == 6
        assert selector_log_lines[0].startswith(f"{tasks_dir}|@")
