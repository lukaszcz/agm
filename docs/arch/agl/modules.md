...
- **Typecheck.** Type identity becomes module-qualified, so same-named types in different modules are distinct. A two-phase pre-pass registers every declared name's handle (or generic template) across all modules first — a handle carries no shape, so forward references and import cycles never block resolution — then resolves every declaration's body in a fixed deterministic order, with no dependency ordering. The structural type-definition dependency graph is still computed to reject a genuine cross-module structural cycle (a type that contains itself by value); this is a temporary restriction, mirrored by each module's own per-module recursion check, pending an inhabitation-based analysis. A function-signature pre-pass resolves every module's signatures before any body is checked, so cross-file mutual recursion does not depend on module checking order.
