...
- **Scope.** A whole-program pre-pass computes per-module export maps — each public path atom mapped to its originating module and declaration path, with re-export chains resolved to their origin — and collects all public declarations before any body is resolved. Per-module collection keys declarations by scope path as well as name, merges repeated scope regions and shorthand declarations, and applies the library declaration restrictions inside regions. Import declarations merge into per-module contributions: each selected set bounds both route access and bare injection; scope-prefix selections expand their public subtrees, and `using` renames re-root selected paths. Plain imports are route-only, while `using` and `open import` add bare candidates, whose full path atoms resolve without a module route. Scope `open` selects local or directly imported public scope subtrees into its enclosing region's shared bare layer, used by both value and type resolution; it does not change the module export map or propagate through importers. Aliases replace plain-path routes, and wildcard declarations distribute each path selection per module. Suffix (`suffix::name`), anchored (`/path::name`), self (`::name`), constructor, and type references all use the same contribution resolver; ambiguities fail at the use site. Non-entry modules are restricted to declarations (no top-level statements, agents, or params).
- **Typecheck.** Function signatures retain declaration paths alongside node identity, so scope members receive the same header collection and candidate inference as root functions. Type identity includes its module, scope path, and name, so same-spelled root and scoped types remain distinct. Because nominal types are shape-free handles ([frontend/types.md](agl/frontend/types.md)), declarations resolve in a fixed order with no dependency ordering — forward references and type cycles spanning modules work exactly like same-module ones, while alias cycles remain illegal. The resolved-program artifact preserves the loader's reverse-topological import SCCs. After collecting explicit, builtin, and extern headers, typecheck traverses those SCCs dependencies first: closed unannotated candidates publish by declaration node id before importers, while an import cycle shares one cross-module function graph. It then authoritatively checks each module body with the completed concrete signature table. The whole-table analyses run once over the shared `TypeTable`.
