...
def test_imported_transformed_generic_alias_matches_concrete_enum_owner(
    ...
    compiled = _compile_graph_case(
        tmp_path,
        {
            ...
            "entry": (
                "import library/remote::{Flipped, make}\n"
                "let value = make()\n"
...
def test_imported_generic_alias_with_fixed_argument_matches_enum_owner(
    ...
    compiled = _compile_graph_case(
        tmp_path,
        {
            ...
            "entry": (
                "import library/remote::{Fixed, make}\n"
                "let value = make()\n"
...
def test_negative_alias_to_other_enum_is_not_selected_as_owner(tmp_path: Path) -> None:
    compiled = _compile_graph_case(
        tmp_path,
        {
            ...
            "entry": (
                "import library/remote::{Wrong, make}\nlet value = make()\ncase value of | _ => 0\n"
            ),
        },
    )
    unavailable = compile_match_site(replace(compiled.normalized, rows=()))
...
def test_reexported_alias_chain_uses_final_exposed_name(tmp_path: Path) -> None:
    compiled = _compile_graph_case(
        tmp_path,
        {
            ...
            "library/facade": "export library/base::{Chained as Public}",
            "entry": (
                "import library/facade::{Public}\n"
...
def test_strong_compiled_case_validator_rejects_internal_corruption() -> None:
    _checked, sites = _compiled_cases(
        ...
        "let pair-value = pair(left = false, right = false)\n"
        "let int-value = 1\n"
        "let box-value: Box = box(value = 1)\n"
        "let int-pair-value = intpair(left = 1, right = 2)\n"
        "let _ = case pair-value of | pair(left = false, right = false) => 1 | _ => 2\n"
        "let _ = case int-value of | _ as captured => captured\n"
        "let _ = case box-value of | box(value = _ as captured) => captured\n"
        "let _ = case int-pair-value of | intpair(left = left, right = right) => left\n"
