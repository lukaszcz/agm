...
- **Coverage** — 100% line and branch coverage of `src/` and of `packages/stdlib/src/` (the standard library's Python companions ship in the wheel; `packages/sysone` does not, and stays outside every gate while its tests still run), measured through `sys.monitoring`, which is why `.python-version` pins the development interpreter to Python 3.14.
...
- **Hermeticity** — a run never reads the machine it runs on: autouse fixtures give each test an empty `HOME` (so the AGM home, config, prompts, and package store are its own) with explicit git identity, strip inherited `AGM_*`, `TYPESAFE_*` (the TypeSafe SDK's), proxy, and project/terminal variables, pin `AGM_STDLIB` to the in-repo `packages/stdlib/`, and report the installation prefix as absent. `tests/test_environment_isolation.py` asserts this against the captured launch environment; tests of the prefix fallback take the `installed_agm_prefix` fixture, and tests needing a populated home pass it explicitly.
...
- **No real network** — an autouse fixture defaults `agm.core.http.open_session` to an empty scripted transport that refuses any request; a test that scripts HTTP (`tests/_http_helpers.py`) overrides it. Another fails the test on any real `httpx2` send (`httpx2.HTTPTransport.handle_request`). The `sysone/jev` companion builds its TypeSafe SDK client through its `open_client` seam; `tests/_jev_helpers.py` reaches the loaded companion through `ExternRegistry.loaded_companion` and swaps the seam for a client over a scripted `httpx2.MockTransport`, which unit tests and the e2e corpus's `jev` scenario key share.
...
- `tests/conftest.py`, `_agl_helpers.py`, `_process_helpers.py`, `_package_helpers.py`, `_git_helpers.py`, `_http_helpers.py`, `_jev_helpers.py` — shared fixtures and fakes.
- `tests/_command_coverage.py`, `tests/_durations.py` — the command-coverage and CPU-cost plugins; `tests/_agm_zygote.py` — the e2e preforking launcher.
