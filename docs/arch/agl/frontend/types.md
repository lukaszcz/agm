...
An enum is a closed nominal union of member record declarations: a value of enum type *is* its selected member record, member-to-enum assignability is directed, and sibling members share no type without an enum annotation. Built-in and host-contract shapes (`ExecResult`, `AgentRequest`, `Option`, the exception hierarchy) use the same table: loaded standard-library declarations are authoritative and are validated against location-independent contracts in `typecheck/builtin_contracts.py`; canonical fallback shapes apply only when no source is loaded. Lowering derives its host nominal table from the same queries, so checking and evaluation cannot pick different nominals.
...
- **Built-in rules** are dispatched from scope's classification: `exec` chooses between a structured result, parsed stdout, and an output-discarding unit contract; `ask` takes its result type from context. Output contracts (schema and format metadata) are computed here and compiled into the typeless layer.
