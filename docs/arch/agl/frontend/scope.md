...
- Constructors live in the value namespace; an ambiguous unqualified constructor name is a static error, disambiguated with `Type::Ctor` qualification. Expression `QualifierChain` nodes preserve that established module-route/type-owner split for one- and two-segment chains; current-module-anchored chains with multiple qualifier segments, and other unsupported future shapes, are rejected before resolution continues.
