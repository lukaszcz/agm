...
- `src/agm/agent/session/` — backend-neutral session protocol, host-owned lifecycle table, and CLI session adapters for persistent agent conversations. Lifecycle and capability errors use `SessionHostError`; a failed `ask` preserves transport diagnostics in `SessionAskError`.
