...
AgL has three value binders (`let`, `var`, `set`), an input declaration, an
...
3. `set` on an immutable binding is an error; the diagnostic names the binder
   kind — `let`, `input`, a catch binder, or a pattern binding.
...
`input` declarations are root-only. Each enters the root scope as an
immutable binding:
...
At run start, before any expression evaluates, the host validates the
provided inputs. A missing input, an undeclared provided input, or a
type-invalid value is a *host invocation error* — not an AgL exception and
not catchable in-language.
