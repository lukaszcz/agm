...
- **Scope.** A whole-program pre-pass computes per-module export maps — each public name mapped to its originating module, with re-export chains resolved to their origin — and collects all public declarations before any body is resolved. Import declarations merge into per-module contributions: plain imports are qualified-only, while `using` and `open import` add bare candidates. Qualified (`suffix::name` or `/anchored/path::name`), self (`::name`), constructor, and type references all use the same contribution resolver; ambiguities fail at the use site. Non-entry modules are restricted to declarations (no top-level statements, agents, or params).
