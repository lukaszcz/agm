...
- Built-in calls (`print`, `exec`, `ask`, and friends) are recognized by resolving the callee to a `builtin def` declaration, not by name alone. Resolution uses lexical lookup, region contributions, and the import environment, so aliases, re-exports, and REPL-retained bindings preserve their original builtin provenance. A module or named-scope member may share a builtin's spelling because it has a qualified namespace, but the spelling remains reserved at the entry module root and bare lookup still selects the builtin; receiver methods likewise require qualification or member selection.
...
Host-backed `builtin var` declarations are admitted in any standard-library
module, at its root or inside a named scope region; every `builtin` form
otherwise rides the same member/duplicate/visibility path as an ordinary
declaration, with no shape-specific placement logic of its own. Only root
`std/config` bindings are engine-setting registers; scoped `std/config` and
other standard-library bindings are ordinary host-backed values.
