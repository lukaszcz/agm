...
Scope-region syntax and declaration-path shorthand cross the parser firewall in canonical AST forms. Collection materializes a `ScopeNode` layer and static member map for each named path, merging repeated regions, shorthand paths, and same-named type scopes. A type's enum variants are collected as members of that type scope, while record and exception construction retains its bare declaration spelling. Ordered qualifier resolution first considers exact local paths through the active scope layers, then constructor compatibility and import routes; the type environment applies the same local/module anchor and clash policy. `::` starts at the module root, and scope paths never suffix-match. Plain scope segments reject type arguments in expression, type, pattern, and `is` chains. Type-owned final members remain on the established constructor checker until their unified migration. Scope `open` and path-aware import/export selection remain rejected before import environments are built.
