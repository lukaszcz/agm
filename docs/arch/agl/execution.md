...
The `guarded` flag marks self-bounded loops — those with a `[n]` bound (which raises `MaxIterationsExceeded` itself) or a `for` clause (bounded by a finite collection). The host's global `max-iters` safety valve (resolved from `--max-iters` / `[exec] max-iters` / a source `std.config::max-iters` write) applies **only to unguarded loops** (`guarded=False`), capping them at `max-iters` body executions. The valve is off by default; its `int`-typed register exposes off as zero, and writing zero disables it. A self-bounded loop is never cut short by the host safety net, which exists solely to catch runaway unbounded `while`/`do…until` loops.
...
Programs are parameterized by executable `param` declarations resolved in order at evaluation time, with precedence external value > default expression > error for a required param. The pipeline can discover a program's parameter inventory before execution so a host can wire external values; this drives the `agm exec` option surface described in [repl.md](repl.md). Params are validated against the lowered program, so a host that must reject bad values before it commits to any run side effect preflights them separately. Every artifact a pass produces — checked, match-compiled, lowered — is therefore handed forward to the next pipeline entry point rather than recomputed, so however many times a host resumes the pipeline, one invocation compiles and lowers the program exactly once.
