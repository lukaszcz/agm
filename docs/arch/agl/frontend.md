...
- **Unified parameter zones and argument binding** — parameters and fields use one node (`Param` with `ParamKind ∈ {POSITIONAL_ONLY, STANDARD, NAMED_ONLY}`). Zone markers (`/`, `*`, `@pos`/`@std`/`@named`) are resolved to per-`Param` kinds in the AST builder; no pass after it ever sees a marker. A single `bind_arguments` routine in `typecheck/arguments.py` performs positional-greedy matching against a kind-annotated parameter list for function calls, constructor calls, and constructor-pattern field binding — the three formerly-separate argument-matching paths now all delegate to it. The bare-name shorthand (`bare VarRef` → `name = name`) applies whenever a positional argument lands on a named-only parameter slot; it is no longer constructor-specific. Constructor field kinds are carried in a registry on the type environment (keyed by owner/variant, with user exceptions registered globally so cross-module construction resolves), and the resolved field order of each constructor pattern is recorded in a checked-program side table so the type-free lowerer can build the match plan without re-deriving it.
