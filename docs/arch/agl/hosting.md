...
Preparation records every `program def` with its module and scope path, so a host can select an entry and bind its own value parameters before execution. The selected entry runs after module initialization inside the interpreter's normal boundary. A module-level `param` declaration still parses and lowers, but no host surface supplies its value: it resolves only from its own source default, and `run_prepared` reports a required `param` with no default (still reachable from the selected program) as a pre-execution diagnostic via `agl/runtime/arguments.py::missing_required_param_diagnostics`. `agm check` uses `check_prepared`, which stops after lowering and skips that check, so a required `param` without a default is accepted there.
...
- `src/agm/agl/runtime/arguments.py` — `program def` argument binding and decoding, and the required-`param` preflight diagnostic; `src/agm/agl/runtime/engine_config.py` — engine-setting decoding/config helpers.
