...
`agm exec` therefore takes its whole tail as one catch-all and derives the FILE argument from it in a single place, rather than letting Click bind the positional. The ordinary derivation recognizes host flags directly; when program flags precede FILE, it resolves candidate sources against the selected program's projected command so flag arity, including boolean and attached short forms, decides which token is FILE. Execution, help, and completion share that derivation. The selected program's command owns `-h`/`--help`, so a help flag is a help request or a value-taking option's value exactly as Click reads it.
