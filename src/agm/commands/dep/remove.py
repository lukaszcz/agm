...
from agm.core.path import display_path, is_portable_relative_path
...
def _parse_target(target: str, *, remove_all: bool) -> tuple[str, str | None]:
    ...
    if remove_all:
        if sep:
            ...
            raise SystemExit(1)
        if not is_portable_relative_path(dep):
            print(f"error: invalid dependency target: {target}", file=sys.stderr)
            raise SystemExit(1)
        return dep, None
    if not sep or not ref:
        ...
        raise SystemExit(1)
    if not is_portable_relative_path(target):
        print(f"error: invalid dependency target: {target}", file=sys.stderr)
        raise SystemExit(1)
    return dep, ref


def _require_descendant(parent: Path, path: Path, *, description: str) -> Path:
    """Resolve *path* and require it to be a strict descendant of *parent*."""

    resolved_parent = parent.resolve(strict=False)
    resolved_path = path.resolve(strict=False)
    if resolved_path == resolved_parent or not resolved_path.is_relative_to(resolved_parent):
        print(f"error: {description} escapes {display_path(parent)}", file=sys.stderr)
        raise SystemExit(1)
    return resolved_path


def _resolve_dependency_path(parent: Path, relative: str) -> Path:
    """Resolve a relative dependency path beneath *parent*."""

    return _require_descendant(
        parent,
        parent.resolve(strict=False) / relative,
...
def run(args: DepRemoveArgs) -> None:
    ...
    dep_dir = _resolve_dependency_path(deps_dir, dep)
    ...
    if args.all:
        for worktree in linked_worktrees:
            _require_descendant(
                dep_dir,
                worktree.path,
                description="dependency worktree",
            )
