...
- **Agent invocation** uses `agm.agent.session`: free and explicit asks own persistent or ephemeral conversation handles, while backend-neutral service operations select the CLI adapter or Pi RPC implementation. CLI asks reuse `agm.agent.runner`; `runtime/sessions.py` bridges AgL values and supplies the dispatcher-backed host used where no session service exists.
