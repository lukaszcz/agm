...
The Python FFI is split between `runtime/externs.py`, which loads companions and dispatches calls, and `runtime/boundary.py`, which converts values at the boundary. Each call has one `BoundaryScope` for seals and its live container views; repeated array or dict occurrences preserve caller-container and Python-view identity only when their boundary schemas are compatible. Equal schemas, and compatible generic/concrete schemas reconciled to the concrete representation, share a view; conflicting concrete shapes and distinct generic seals do not. A bare generic occurrence is a sealed handle, not a view. All views are revoked when the call ends. Encoding an array or dict argument creates a view rather than walking the container, so it has no encode-side container cycle guard.
