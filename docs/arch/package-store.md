...
Archives (`.agmpkg`) are deterministic ZIP files. Readers bound metadata, entry sizes, total expansion, and path depth, and bind preflight, parsing, verification, and extraction to one opened file. URL dependency fetches run through the shared HTTP transport seam ([core.md](core.md)), which streams the archive to a temporary file while digesting and size-capping it; the fetcher compares the digest, translates transport failures into its own error vocabulary, and hands the file to the installer. They are bounded by transfer inactivity and download size rather than total elapsed time; a signal-based watchdog additionally bounds reaching the archive's first byte -- where name resolution and connect block outside any timed socket read -- and is dropped once the body starts arriving, so a slow link is never preempted.
