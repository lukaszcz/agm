...
- Built-in calls (`print`, `exec`, `ask`, and friends) are recognized by resolving the callee to a `builtin def` declaration, not by keyword. Resolution is the ordinary one — lexical lookup, region contributions, and the import environment alike — so a bare call is reached through the standard library's own declaration exactly as a qualified one is reached through its path, and there is no route to a built-in that bypasses a declaration.
