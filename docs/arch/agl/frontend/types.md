...
- **Function headers** are resolved through one shared `typecheck/function_inference.py` seam for standalone and program checking. Per-module checking routes its disposable direct-recursion candidate pass for unannotated monomorphic functions through a synthetic singleton module component, leaving an extension boundary for later import/SCC coordination. Candidate common-type evidence from branches, early returns, body tails, and literal elements is normalized before provisional results are solved. Bottom literal evidence is discarded, so it neither conflicts with concrete evidence nor closes an otherwise unresolved candidate; ordinary equality and `int`-to-`decimal` widening remain independent of traversal order. Known call and constructor shapes supply exact provisional evidence; shape-preserving and fixed-result operations, including membership, can continue through unresolved operands, while unknown field/index shapes do not guess an owner or container. The ordinary checker then rechecks the body against the concrete candidate. Generic functions stay on the ordinary path so recursive generic results require an explicit annotation. Candidate node types, calls, contracts, bindings, and warnings are discarded. Program pre-passes retain named internal signature metadata keyed by declaration node id; public `FunctionSignature` and `FunctionType` remain concrete semantic types.
