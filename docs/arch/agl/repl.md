...
`:info NAME` reads the retained session scope and type environment without parsing or evaluating source. It reports live bindings, function signatures, and retained nominal, generic, or alias types as concise AgL fragments; the rich console routes that result through the same live lexer and theme as input, while plain output remains text.
