...
`import` adds a graph edge and qualified access to a module's public surface; import tails and `use` add bare names to a region without narrowing that surface; `hiding` subtracts paths. Reachable qualified declaration routes also determine method visibility, while `use` never loads a module or makes methods visible. Imports, uses, and exports may appear inside `scope` regions: qualified routes stay module-wide, bare contributions are regional, and a scoped export re-roots forwarded paths beneath the region. Re-exports are part of the same graph. A simple static `let`/`var` is exported like any other declaration, and a cross-module reference to it keeps `var`'s mutability.
...
Every loaded module except `std/prelude` receives the `std/prelude` prelude unless the host disables it or the module imports `std/prelude` explicitly. The prelude is only that auto-imported entry point (`packages/stdlib/src/prelude.agl` holds export lines): it re-exports the scopes that declare builtin-receiver methods, making those methods visible through ordinary import routes wherever the prelude is loaded. The host recognizes the *standard* declaration of a built-in name as the `builtin` declaration of that name in any standard-library module, wherever it lives, and a `builtin` declaration outside the standard library overrides it. Built-in names nothing declares — under `--no-stdlib`, say — fall back to fixed reserved identities that belong to a sentinel module rather than to any file, and spell bare wherever a name is rendered. Standard-library modules may declare host-backed `builtin var` bindings, identified by module, scope path, and name; only root `std/config` bindings are engine settings. A module may instead declare an ordinary `@param` binding as a module-owned setting reachable like any module parameter — `std/log`'s `level` and `std/http`'s `timeout` (`--http-timeout`) are both this shape, not `builtin var`. Modules with `extern def` declarations have Python companions ([execution/ffi.md](execution/ffi.md)); the companion-backed surface spans collections, text, JSON, TOML, math, paths, URLs, HTTP, filesystem, process, time, random, and regex. Library conventions — every AgL-visible name is kebab-case (Python companion attributes and host-side keys stay snake_case), a filesystem location is typed `path` and a URL is typed `url` (each a transparent `text` alias its own module declares and the prelude forwards on its own, without the module's functions), an exception type is declared beside the operations raising it, a raising operation's `?` twin returns `Option` and its `try-` twin returns `Result` with the specific exception type, `!` marks in-place operations, and Option-valued operations with no raising form carry no suffix — are stated for users in `docs/agl/reference/modules.md`; `try-` twins are pure AgL over the raising operation, never separate externs.

`std/optional` declares the built-in `Optional[T]` enum by extending `Option[T]`'s
shared `Some`/`None` members with the inline host-default sentinel `Default`; the prelude
exports both modules.

Text construction also has a pure AgL builder in `packages/stdlib/src/text.agl`: a record backed by the existing mutable array operations and joined into immutable text snapshots. It follows ordinary record/collection aliasing and copying semantics without introducing another runtime value kind.
...
`modules/parsed_module_cache.py` assigns content-addressed node namespaces so unchanged declarations keep their identity across import orders and processes. `artifact_cache.py` validates each stage against the module and its transitive import/export dependencies; checked stages also depend on host capabilities. Modules sharing a dependency closure share a persisted frontend image. Restored artifacts attach to the current compilation's source objects, preserving stage provenance. A checked module's disk artifact carries its own declarations plus a journal of the mutations its body check made on its type environment, never the whole-program environment it read from; restoring it replays that journal onto the environment the current compilation prepares. In-process reuse keeps the full checked object.
...
- `packages/stdlib/src/` — the standard library.
