...
- **Function headers** are resolved through one shared `typecheck/function_inference.py` seam for standalone and program checking. Program pre-passes retain named internal signature metadata keyed by declaration node id; public `FunctionSignature` and `FunctionType` remain concrete semantic types.
