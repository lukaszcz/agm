...
Companions are imported after the static passes succeed and before evaluation starts, so a broken companion is a load-time diagnostic and `--dry-run` never imports one. The registry reuses an imported companion while its canonical path and file identity stamp are unchanged. Its bytecode is cached in the AgL artifact store ([modules.md](../modules.md)) under that path and stamp plus compiler digest and optimization level, never in a `__pycache__` beside the source, since a companion may live in an installed package's immutable store tree; a stamp too recent to rule out a same-tick edit is not cached. A host capability flag gates the FFI the way another gates shell `exec` ([hosting.md](../hosting.md)). The name a companion callable is looked up under is scope's `extern_names` fact ([frontend/scope.md](../frontend/scope.md)).
