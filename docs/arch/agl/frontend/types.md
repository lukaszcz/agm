...
- **Member selection and assignment targets** share one field/method decision: a field remains selectable only when no visible same-named method exists; a clash is static ambiguity, while a methods-only assignment target reports that methods are not assignable. Exception methods select from the nearest visible inheritance level. Declaration validation rejects a local method that shares its owner or an ancestor's field, but a descendant field is selected only at use sites. Assignment targets otherwise are rebinding, container index, or record `var` field; a field assignment needs a precise record receiver, so an enum must be narrowed to its member first.
