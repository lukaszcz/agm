...
class TestBlockTyping:
    ...
    def test_set_at_end_is_valid(self) -> None:
        # SetStmt at end is valid (it produces unit, not a LetDecl)
    ...
    def test_input_declaration(self) -> None:
        r = accept_type("input x\nx")
        assert r.resolved.program is not None

    def test_input_with_annotation(self) -> None:
        r = accept_type("input n: int\nn")
        assert r.resolved.program is not None

    def test_input_defaults_to_text(self) -> None:
        r = accept_type("input x\nx")
        prog = r.resolved.program
        input_decl = prog.body.items[0]
        assert isinstance(input_decl, InputDecl)
        binding_type = r.type_env.get_binding_type(input_decl.node_id)
...
class TestUnitPropagation:
    ...
    def test_set_is_valid_block_item(self) -> None:
        r = accept_type("var x = 1\nset x = 2\n()")
        assert r.resolved.program is not None
...
class TestVarSet:
    ...
    def test_set_type_mismatch_raises(self) -> None:
...
class TestMisc:
    ...
    def test_input_no_annotation_is_text(self) -> None:
        r = accept_type("input x\nx")
        decl = r.resolved.program.body.items[0]
