...
class TestDirectASTConstruction:
    ...
    def test_input_inside_if_rejected(self) -> None:
        err = reject_scope(
            "if true =>\n"
            "  input late\n"
            "| else =>\n"
    ...
    def test_input_inside_do_rejected(self) -> None:
        err = reject_scope("do[2]\n  input x\nuntil true\n")
        line, msg = diag(err)
        assert line == 2
        assert "input" in msg.lower()

    def test_input_inside_try_rejected(self) -> None:
