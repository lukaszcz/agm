...
- Built-in calls (`print`, `exec`, `ask`, and friends) are recognized by resolving the callee to a `builtin def` declaration, not by name alone. Resolution uses lexical lookup, region contributions, and the import environment, so aliases, re-exports, and REPL-retained bindings preserve their original builtin provenance. A receiver method may share a builtin's name but is reached only through qualification or member selection; it does not intercept the bare builtin call. Scope records type-scoped static candidates from the canonical prelude path; typecheck accepts `Session::open`/`default` headers only for the exact prelude nominal identity, and routes `Session` host methods by that same exact receiver identity. Neither rule falls back to a matching name or shape. An unrelated nested `User::Session` neither shadows those root statics nor changes its own fully-qualified enum/record construction; final segments remain ordinary identifiers and same-spelled user enum constructors continue through the existing qualified-constructor path.
