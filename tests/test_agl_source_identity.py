...
class TestAglSyntaxErrorWithSource:
    ...
    def test_transformer_error_with_source_has_label(self) -> None:
        ...
        Regression: errors raised in the transformer (e.g. "Unknown generic
        type") were unwrapped from VisitError and re-raised WITHOUT the source
        stamp, so they reported ``<agl>`` instead of the module path.
        """
        sid = SourceId(label="/mods/broken.agl")
        with pytest.raises(AglSyntaxError) as exc_info:
            # "whatever[int]" triggers an "Unknown generic type" error inside
            # the AstBuilder transformer, wrapped in a VisitError.
            parse_program("def f() -> whatever[int] = 1", source=sid)
        err = exc_info.value
        assert err.span is not None
