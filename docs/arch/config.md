...
- **Engine settings** (`default-agent`, `strict-json`, `timeout`, `trace`, `trace-file`) are `builtin var` bindings at the root of `std/config`. Precedence is source write > CLI flag > qualified program table > selected program's own `@config` entries > `[exec]` > engine default. Every key, `default-agent` included, decodes into its typed seed value through the same host-value decode path. Their catalog — name, value kind, config accessor, default, which side of the host boundary consumes a write, which host register a key shares, and whether its value is a path — is the pure data leaf `config/engine_keys.py`, shared with the AgL checker, IR validation, and evaluator. Every projection derives from it: which config fields resolve as paths, which CLI flags are mutually exclusive (`cli_support/run_options.py`), and the `trace-file` ⇒ `trace` implication a runtime write follows. `cli_support/engine_seeds.py` resolves the CLI-over-config tiers once per run, and `agm exec` and `agm repl` take both the seeded `trace` setting and the trace file they open from that one resolution, so the two can never disagree.
