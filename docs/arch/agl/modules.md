...
- **Typecheck.** Type identity becomes module-qualified, so same-named types in different modules are distinct. A two-phase pre-pass registers every declared nominal name's handle (or generic template) across all modules first — a handle carries no shape, so forward references and import cycles never block resolution — then resolves every declaration's body in a fixed deterministic order, with no dependency ordering: body resolution only produces nominal handles, so it never needs referenced nominal types built first, and a type cycle may span any set of modules exactly as a same-module cycle can. Transparent type aliases have no handle shell, so graph environments resolve cross-module aliases lazily when a type body references them and still reject alias cycles. Once every body across the graph is resolved, the inhabitation analysis described in [frontend.md](frontend.md) runs once over the shared `TypeTable`, rejecting the first uninhabited declaration (by sorted key, for determinism) regardless of which modules it spans; the finiteness and equality-capability analyses over the same table are computed lazily at their use sites and cached. A function-signature pre-pass resolves every module's signatures before any body is checked, so cross-file mutual recursion does not depend on module checking order. Each checked module retains the immutable import environment used to construct its type environment, making exact source aliases and exposure controls available to later checked-program consumers without private-state access. Public source-template lookup resolves QNames through checked alias chains into immutable semantic templates. Public enum-owner forms then pair those templates with the exact local, self, open-import, or qualified-import spelling admitted by that environment, preserving ambiguity and cycle rules while hiding mutable registries.
