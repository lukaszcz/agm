...
- `src/agm/agent/session/` — backend-neutral session protocol, host-owned lifecycle table, CLI continuation adapters, and Pi's persistent RPC adapter. CLI adapters cover persistent AgentCommand, Claude, Codex, and Pi conversations; the RPC adapter owns a bounded, strict JSONL Pi child transport, waits for command acknowledgements and settled prompt events, reports terminal prompt failures, and terminates the child on close or protocol failure. Pi verifies an independently opened parent before cloning an active RPC branch, then transactionally assigns independent parent and child processes; Claude/Pi CLI adapters keep generated session ids. Codex validates initial `--json` creation output to capture `thread.started` and its completed assistant text, then resumes that thread with normal plaintext output. Session creation state is consumed before its first transport call, so retries resume safely and `reset` is the explicit way to create again. Lifecycle and capability errors use `SessionHostError`; a failed `ask` preserves transport diagnostics in `SessionAskError`.
