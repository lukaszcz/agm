...
- **Agent invocation** goes through `agm.agent.runner` (the same prepare/run path as loop/review). Each `ask` evaluates an `Agent`-typed member `RecordValue`, which `runtime/agents.py` decodes into the corresponding runner argv; an omitted argument reads `std/config::default-agent`, seeded by `exec`/`repl` configuration or `--agent`.
