"""Resolution of the active managed standard-library package root.

This is package-domain work — activation lookup, canonical identity, store
paths, manifest loading, and ``RECORD`` verification — so it lives here
rather than in the configuration layer. ``agm.config.module_roots`` handles
only the ``AGM_STDLIB`` environment override and delegates everything else to
...
from agm.packages.model import STD_PACKAGE_NAME, canonical_package_identity
...
def resolve_std_package_root(*, home: Path, env: Mapping[str, str] | None = None) -> Path:
    """Return the selected AgL standard-library module root.

    An active immutable store ``std`` package wins only when its version
