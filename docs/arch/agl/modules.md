...
- **Match compilation.** Every source `case` and immutable `let` in every checked module is compiled, including sites entry code never calls. The program artifact carries a total per-module mapping; case coverage/redundancy and refutable-let issues prevent lowering and are reported with their originating source labels. The current lowerer consumes only case decisions; let decisions remain static proof artifacts.
