...
- **Runtime-live** (`strict-json`, `max-iters`, `timeout`) update live interpreter fields (`_strict_json`, `_loop_limit`, `_shell_exec_timeout`), so the effect takes hold from the write onward for subsequent agent-output parsing, unbounded-loop checks, and `exec` evaluation, respectively. The timeout register separately retains its assigned `Option[text]` value so reads preserve the original duration spelling while execution uses parsed seconds.
- **Host-consumed** (`runner`, `log`, `log-file`) update an in-interpreter register and reconfigure the live host service on write (`_reconfigure_host_service`, `runtime/host_settings.py`): a `runner` write rebuilds the default agent used by later unnamed calls and declared agents without a dedicated backing; `log`/`log-file` writes repoint the best-effort trace destination. Runner rebuilding is transactional with its register. Trace-path I/O failures instead disable tracing for the rest of the execution (the whole REPL session in interactive mode) without rolling back the assigned setting.

Because a write is an ordinary positional statement, settings take effect in program order and `agm exec` runs the program in a **single phase** — there is no separate startup pass. The CLI and config-file layers seed the initial register/field values (see [config.md](../config.md)); a source write then overrides them from its point onward. In the REPL a completed write persists across entries even when a later initializer in the same entry fails, matching the session's non-transactional runtime and partial static promotion; `:reset` restores the seeded defaults. Built-in setting stores use the same trace mutation event as ordinary `:=` stores.
