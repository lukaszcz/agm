...
- **Engine settings** (`default-agent`, `default-sandbox`, `strict-json`, `timeout`, `trace`, `trace-file`) are `builtin var` bindings at the root of `std/config`. Precedence is source write > CLI flag > qualified program table > selected program's own `@config` entries > `[exec]` > engine default. Every key, including `default-agent` and `default-sandbox`, decodes into its typed seed value through the same host-value decode path. Their catalog — name, value kind, config accessor, default, which side of the host boundary consumes a write, which host register a key shares, and whether its value is a path — is the pure data leaf `config/engine_keys.py`, shared with the AgL checker, IR validation, and evaluator. Every projection derives from it: which config fields resolve as paths, which CLI flags are mutually exclusive (`cli_support/run_options.py`), and the `trace-file` ⇒ `trace` implication a runtime write follows. `cli_support/engine_seeds.py` resolves the CLI-over-config tiers once per run, and `agm exec` and `agm repl` take both the seeded `trace` setting and the trace file they open from that one resolution, so the two can never disagree.
...
- **Module parameters** — marked static `let`/`var` bindings — resolve as CLI flag > `@opt-env` environment variable > program route > selected program's own `@config` entries > module route > initializer. `param_surface.py` resolves their external bare and dotted spellings against host flags, signature parameters, the entry module, and imports; `param_config.py` reads module/scope tables and the program route independently, so the latter always wins. The program route accepts every spelling a parameter wins, bare or dotted, carried as one key's alternative leaves so the ordinary layering rules apply: several spellings in one layer conflict, a later layer overrides. The REPL uses the module route only when it first loads a module.
- **`@config`** attaches key/value entries directly to a `program def`, evaluated once that program is selected and lowered (never for `agm check`, the REPL, or a call to the program as an ordinary function). Its keys are ordinary refs to a module parameter or an engine setting, so the same entry set can target either kind; the host partitions them by target and folds each into the precedence chain above at its own rank. See [agl/hosting.md](agl/hosting.md).
