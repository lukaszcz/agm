...
The semantic type model lives in the `semantics` foundation package and is consumed by the typecheck pass. Alongside the ordinary scalar, container, record, and enum types it carries the types the expression-oriented design needs: a unit type for side-effecting expressions, a positional function type, an opaque agent type, a bottom type for `raise`, and rigid type variables for generics. `RecordType`/`EnumType` are lightweight handles — `(module_id, name, type_args)` — carrying no field/variant data of their own; nominal identity is by name and owning module (in graph mode), not by structure, and is exactly the handle's identity tuple.

`semantics/type_table.py` holds a `TypeTable`: a registry of `TypeDef`s (a declaration's type parameters and field/variant type templates) keyed by `(module_id, name)` — the sole source of record/enum field and variant shapes. The type builder registers a `TypeDef` for every user-declared record and enum as its body is resolved, and a module graph's per-module environments share one instance. Built-in prelude shapes (`ExecResult`, `ParsePolicy`, `OutputContract`, `OutputContractOption`, `AgentRequest`, and the generic `Option`) are explicit `TypeDef` literals in the same module, the single definition shared by table seeding, builtin-shape validation, and scope-pass constructor-candidate seeding. Because handles carry no shape, every field/variant type — the typecheck pass (member access, enum pattern typing and exhaustiveness, constructor checking), the table-aware `comparable_types` it also hosts, compile-time schema derivation (`type_schema.py`), and lowering (constructor construction, nominal descriptors, coercions, conversions) — is looked up through the table by handle. Exceptions are not yet registered in the table and still carry their fields embedded.

Recursive record/enum/exception declarations are rejected: a declaration that structurally contains itself (directly or through other declarations) is a static error, checked after all of a module's declaration bodies are resolved. This is a temporary restriction pending an inhabitation-based analysis that will allow well-founded recursion.
