...
- **Agent invocation** routes free `ask` through a snapshot `Session::default`, whose agent comes from `std/config::default-agent` seeded by `exec`/`repl` configuration or `--agent`. `Agent::ask` instead opens one ephemeral session for its complete retry loop. The AGM adapter decodes agent values and selects the runner/backend.
