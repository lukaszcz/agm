...
- **Typecheck.** Type identity becomes module-qualified, so same-named types in different modules are distinct. A two-phase pre-pass registers every declared name's handle (or generic template) across all modules first — a handle carries no shape, so forward references and import cycles never block resolution — then resolves every declaration's body in a fixed deterministic order, with no dependency ordering: body resolution only produces handles, so it never needs referenced types built first, and a type cycle may span any set of modules exactly as a same-module cycle can. Once every body across the graph is resolved, the inhabitation analysis described in [frontend.md](frontend.md) runs once over the shared `TypeTable`, rejecting the first uninhabited declaration (by sorted key, for determinism) regardless of which modules it spans; the finiteness and equality-capability analyses over the same table are computed lazily at their use sites and cached. A function-signature pre-pass resolves every module's signatures before any body is checked, so cross-file mutual recursion does not depend on module checking order.
