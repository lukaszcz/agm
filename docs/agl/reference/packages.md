...
it. A command-backed program takes no value or type parameters and declares an
explicit `-> unit` result; it remains an ordinary callable function inside the
package.
...
A registered command runs its program exactly as `agm exec` would: the params
of the program's module graph become flags, and configuration is read by
qualified key.

## Parameters and configuration keys

Package modules declare [`param`](bindings-and-scope.md) bindings like loose
modules; a program receives values for every param in its module and its
transitive imports, across package boundaries. Because a package module keeps
its package-qualified route, the qualified configuration table and the
module-qualified flag spelling of a param use that route: a `review::max-tries`
param in `review-tools/judge` reads `[judge.review]` when that suffix is
unambiguous, and an exact quoted route such as `["review-tools/judge".review]`
disambiguates. The suffix rules are those of the
