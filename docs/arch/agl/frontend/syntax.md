...
- Declaration nodes carry an optional attribute prefix — a name plus ordinary call arguments — held verbatim. The AST makes no claim about which attributes exist or what they mean; recognition happens against the catalog in `agl/attributes.py`, a dependency-free leaf the AST itself never imports. Parameter zones are the one attribute meaning the parser resolves itself: `Param.kind` is filled from the `@arg-*` attribute an entry or its owning declaration carries (falling back to the form's default zone), and an entry list written out of zone order is a syntax error.
