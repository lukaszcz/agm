...
- **Commands orchestrate; primitives do.** Command modules wire config, project layout, git, and agents together. Reusable behavior lives in `core/`, `project/`, `vcs/`, `config/`, and `agent/`, never copied into individual commands.
- **Configuration is layered TOML.** Settings merge across install, home, project, and workspace scopes; per-command sections override base sections; AgL source pragmas and CLI flags override the file layers for the relevant commands.
...
- **AgL is firewalled and self-contained.** Its passes depend only on a stable AST, never on the parser; it is isolated from the rest of AGM behind the `exec`/`repl` commands and lazily imported.
