...
The REPL is a UI-free incremental driver that runs the full pipeline one entry at a time against two persistent images: the static session environment and the linked IR/runtime image. Node ids stay globally unique across entries, new references fall through to prior session bindings, retained named scope layers keep path-qualified declarations available (for example `A::Token`), and new declarations shadow. Each entry contributes a delta of initializers executed against one persistent base frame, so earlier initializers and host calls are never replayed.

Every entry is an inference boundary. Before promotion, its unannotated functions use the same disposable candidate-return discovery as a standalone module, then ordinary checking validates the resulting concrete signatures. Only validated concrete signatures and final checked artifacts can enter the session; a static failure promotes neither, so later entries start clean. Runtime failure is deliberately non-transactional — initializers completed before a failure remain installed, and the static environment advances only for the symbols that actually reached the runtime frame. A trailing binder keeps the language's unit block result (or bottom when its initializer exits), while the REPL presentation continues to echo the stored binding's name, type, and value; check-only reports bottom for an initializer that always exits. No REPL-only syntax is synthesized, and multi-item entries retain ordinary block discard checking. Redefining a nominal type invalidates prior bindings whose static type mentions it, in retained named-scope layers as well as at the root, and the invalidated members are excluded from the ambient carry-forward rather than resurrected by it. Match compilation runs before the check-only early return, so non-exhaustive cases cannot bypass static validation.

The REPL uses the program pipeline ([modules.md](agl/modules.md)) by default so it gets the same automatic `std/core` open import as `agm exec`, in each entry and its loaded library modules; `agm repl --no-stdlib` disables that injection throughout every loaded REPL program, including after `:reset`. Library modules are cached and incrementally linked. Imports persist as one retained generation per entry, kept as written so a retained wildcard re-expands each entry and tracks modules added since. Precedence is by resolved module identity: a later generation replaces every prior declaration for each module it names, while declarations for one module in the new entry retain normal batch union semantics. Retained scope `open` items are replayed in later entries and discarded by `:reset`. This keeps selected-set, suffix, anchored, alias, open, `using`, and `hiding` behavior aligned with batch resolution. User `infixl`/`infixr` fixity also persists by accumulation and replay. Type-focused surfaces (`:type`, check-only echoes) use a REPL-specific type formatter that expands records and enums to declaration-like listings while keeping ordinary echoes compact. An entry is classified by its last item, and a declaration echo names it as the session makes it reachable: a scoped declaration by its path-qualified spelling, a scope region by the path it opened.
