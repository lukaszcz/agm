...
Lowering emits each module's initializer sequence through one shared walk — function closures first, then the remaining items in source order — and records, for each emitted initializer, the source item it came from. That mapping is the lowering layer's published answer to "which declaration does this initializer complete"; no later pass re-derives it from the AST.
