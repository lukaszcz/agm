...
- **Hermeticity** — a run never reads the machine it runs on: autouse fixtures give each test an empty `HOME` (so the AGM home, config, prompts, and package store are its own) with explicit git identity, strip inherited `AGM_*`, `TYPESAFE_*` (the TypeSafe SDK's), and project/terminal variables, pin `AGM_STDLIB` to the in-repo `packages/stdlib/`, and report the installation prefix as absent. `tests/test_environment_isolation.py` asserts this against the captured launch environment; tests of the prefix fallback take the `installed_agm_prefix` fixture, and tests needing a populated home pass it explicitly.
