...
`agm exec` therefore takes its whole tail as one catch-all and derives the FILE argument from it in a single place, against the host's own flag inventory, rather than letting Click bind the positional; execution, help, and completion all read the same derivation. The selected program's command owns `-h`/`--help`, so a help flag is a help request or a value-taking option's value exactly as Click reads it, and there are no heuristics on either side.
