...
Use Sonnet subagents to implement the tasks. When handing off each task, include a brief summary of the ultimate goals of the whole plan. Make it clear that any work done must be a step toward these goals. Write the subagent task description as a file under .agent-files/tasks/*.md that the implementer can reference. Launch subagents sequentially one at a time, not in parallel.
