...
Use Sonnet subagents to implement the tasks. When handing off each task, include a brief summary of the ultimate goals of the whole plan. Make it clear that any work done must be a step toward these goals, but they are not acceptance criteria for the task. Provide the task description directly in the subagent request. Launch subagents sequentially one at a time, not in parallel.
