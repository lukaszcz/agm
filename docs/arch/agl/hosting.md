...
Preparation records every `program def` with its module and scope path, so a host can select an entry and bind its own value parameters before execution. The selected entry runs after module initialization inside the interpreter's normal boundary. `agm check` uses `check_prepared`, which stops after lowering, so it never binds or validates a selected program's own value parameters — a required parameter with no default is accepted there, unlike under `agm exec`.
