...
- **Engine settings** (`default-agent`, `default-sandbox`, `strict-json`, `timeout`, `trace`, `trace-file`) are `builtin var` bindings at the root of `std/config`. Precedence is source write > CLI flag > qualified program table > selected program's own `@config` entries > `[exec]` > engine default. Every key decodes into its typed seed value through the same host-value decode path; an enum-backed key such as `default-agent` (`Agent`) or `default-sandbox` (`AgentSandbox`) restamps that value onto whichever nominal table is reading it. Their catalog — name, value kind, config accessor, default, and which side of the host boundary consumes a write — is the pure data leaf `config/engine_keys.py`, shared with the AgL checker, IR validation, and evaluator.
