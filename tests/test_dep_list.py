...
class TestReadDepsFromToml:
    def test_reads_valid_toml(self, tmp_path: Path) -> None:
        ...
        result = dep_list_cmd._read_deps_from_toml(config_file)
    ...
    def test_handles_unreadable_file(self, tmp_path: Path) -> None:
        result = dep_list_cmd._read_deps_from_toml(tmp_path / "nope" / "config.toml")
        assert result == {}
...
class TestListAllDepCheckouts:
    ...
    def test_lists_checkouts_per_dep(self, tmp_path: Path) -> None:
        deps_dir = tmp_path / "deps"
...
class TestListDepsAll:
    def test_lists_all_checkouts(
        ...
        dep_list_cmd.list_deps(all_checks=True)
    ...
    def test_all_verbose_shows_path(
        ...
        dep_list_cmd.list_deps(verbose=True, all_checks=True)
...
class TestDepListViaCli:
    def test_dep_list_via_cli(self, monkeypatch: pytest.MonkeyPatch) -> None:
        runner = CliRunner()
        calls: list[dict[str, bool]] = []

        def record(*, verbose: bool = False, all_checks: bool = False) -> None:
            calls.append({"verbose": verbose, "all_checks": all_checks})

        monkeypatch.setattr(cli.dep_list_command, "run", record)
        result = _invoke(runner, ["dep", "list"])
        assert result.exit_code == 0
        assert len(calls) == 1
        assert calls[0]["verbose"] is False
        assert calls[0]["all_checks"] is False

    def test_dep_list_verbose_via_cli(self, monkeypatch: pytest.MonkeyPatch) -> None:
        runner = CliRunner()
        calls: list[dict[str, bool]] = []
        ...
        monkeypatch.setattr(cli.dep_list_command, "run", record)
        result = _invoke(runner, ["dep", "list", "-v"])
        assert result.exit_code == 0
        assert len(calls) == 1
        assert calls[0]["verbose"] is True
        assert calls[0]["all_checks"] is False

    def test_dep_list_all_via_cli(self, monkeypatch: pytest.MonkeyPatch) -> None:
        runner = CliRunner()
        calls: list[dict[str, bool]] = []
        ...
        monkeypatch.setattr(cli.dep_list_command, "run", record)
        result = _invoke(runner, ["dep", "list", "--all"])
        assert result.exit_code == 0
        assert len(calls) == 1
        assert calls[0]["verbose"] is False
        assert calls[0]["all_checks"] is True

    def test_dep_list_verbose_and_all_via_cli(self, monkeypatch: pytest.MonkeyPatch) -> None:
        runner = CliRunner()
        calls: list[dict[str, bool]] = []
        ...
        monkeypatch.setattr(cli.dep_list_command, "run", record)
        result = _invoke(runner, ["dep", "list", "-v", "--all"])
        assert result.exit_code == 0
        assert len(calls) == 1
        assert calls[0]["verbose"] is True
        assert calls[0]["all_checks"] is True
