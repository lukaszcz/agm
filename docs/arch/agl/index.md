...
- **Configuration** is loaded and layered by `agm.config`. The program's own engine settings are `builtin var` bindings in the standard-library module `std/config`; an optional constant initializer supplies the default only when no host seed is present. `constant.py` offers a typed-text constant parser for host adapters: it parses one AgL expression, typechecks it against the declared setting type, and evaluates only constructors and literals into an ordinary runtime value. No production adapter consumes this facility yet. The evaluator reads and writes settings through `IrBuiltinLoad`/`IrBuiltinStore`, backing runtime-live settings with live interpreter fields and host-consumed settings with registers that configure the live host service at startup and on writes (see [repl.md](agl/repl.md)). The `exec`/`repl` commands seed the initial values from the CLI and config-file layers using shared helpers (`core.log`); a source write overrides them from its program point onward.
