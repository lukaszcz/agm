...
- **End-to-end acceptance.** Whole-program suites for module and multi-file AgL programs are part of the standing gate and must stay green. Their scenario harness scripts agent and shell boundaries, including deterministic session tags, exact observed session sets when supplied, separately scripted session capabilities and ordered operation outcomes, and exact per-session prompt and operation counts when asserted, so workflow coverage remains deterministic without executing real agents or shell commands.
