...
Use Sonnet subagents to implement the tasks. Launch subagents sequentially one at a time, not in parallel. Choose the next task based on .agent-files/tasks/PROGRESS.md.
...
After each implementation agent finishes, use an Opus subagent to review its work for correctness, completeness, maintainability, adherence to the task file and to relevant AGENTS.md files.
