...
- `src/agm/agent/session/` — the backend-neutral session protocol and host lifecycle service, CLI continuation adapters, and Pi's persistent RPC adapter. Claude and Pi share generated-session-id lifecycle state while retaining backend-specific argv and fork behavior; Codex captures its created thread before resuming it. See `protocol.py`, `service.py`, `cli_adapters.py`, and `rpc.py` for the respective boundaries.
